...
public class Company {
	...
	*/
	...
	/**
 	...
 	@return string list of employees in the order they were added in
	*/
	...
	alphabetical order.
	@return string list of employees in the order of their departments
	*/
	...
	@return string list of employees in chronological order of when they were hired
	*/
	public String printByDate() { //print earning statements by date hired
		...
		if(numEmployee != 0) {
			...
			for(int j=0; j < emplist.length; j++) {
				...
				{
					builder.append(emplist[j].toString() + "\n");
				}
			}
			return builder.toString();
		}
	...
	/**
	Helps export the current database. 
	@return string literal of the state of the source file.
	*/
	public String exportDatabase(File sourceFile) {
		try {
			...
			PrintWriter printw = new PrintWriter(sourceFile);
			
			for(int n = 0; n < numEmployee; n++) {
				printw.println(print());
			}
			printw.close();
			scan.close();
